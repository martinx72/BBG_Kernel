...
struct rk817_codec_priv {
	...
	struct gpio_desc *avrman_spk_con_gpio;
	struct gpio_desc *avrman_hp_det_gpio;
...
static struct rk817_codec_priv *avrman_rk817;
static int hp_irq_flag = 0;

int headphone_status;

int avrman_suspend = 0;
int bomb_sound = 0;
		
void avrman_set_spk_con_gpio( int gpio )
{
	if( gpio){
		headphone_status = 0;
		gpiod_set_value( avrman_rk817->avrman_spk_con_gpio,1);
	}
	else{
		headphone_status = 1;
		gpiod_set_value( avrman_rk817->avrman_spk_con_gpio,0);
	}
}
...
{
	int ret;
	int irq = gpiod_to_irq( avrman_rk817->avrman_hp_det_gpio );
	unsigned int type;

	type = gpiod_get_value( avrman_rk817->avrman_hp_det_gpio ) ? IRQ_TYPE_EDGE_FALLING : IRQ_TYPE_EDGE_RISING;
	...
	if(1 == gpiod_get_value( avrman_rk817->avrman_hp_det_gpio )){
		avrman_set_spk_con_gpio(0);
	}else if(0 == gpiod_get_value( avrman_rk817->avrman_hp_det_gpio )){
		avrman_set_spk_con_gpio(1);
	}
	enable_irq(irq);
}
...
irqreturn_t hp_det_irq_handler(int irq, void *dev_id)
{
...
static int bomb_sound_function(void *data)
{
		mdelay(50);
		bomb_sound = 0;
		avrman_set_spk_con_gpio(1);
...
//	case RING_HP_NO_MIC:
		...
		if( bomb_sound == 1 )
		{
			...
			tsk = kthread_run(bomb_sound_function, NULL, "bomb_sound thread");
		}			
...
static int rk817_probe(struct snd_soc_component *component)
{
	...
	rk817_playback_path_config(component, OFF, HP_PATH);
					 ...
					 struct rk817_codec_priv *rk817)
{
	...
	rk817->avrman_hp_det_gpio = devm_gpiod_get_optional(dev, "avrman-hp-det",
						  GPIOD_IN);

	rk817->avrman_spk_con_gpio = devm_gpiod_get_optional(dev, "avrman-spk-con",
						  GPIOD_OUT_LOW);

	hp_irq = gpiod_to_irq( rk817->avrman_hp_det_gpio );

	if (hp_irq) {
		ret = devm_request_threaded_irq(dev, hp_irq, NULL,
						hp_det_irq_handler,
						IRQ_TYPE_EDGE_BOTH |
						IRQF_ONESHOT,
						"headphone detect", rk817);
		if (ret < 0) {
			return ret;
		}
	}
