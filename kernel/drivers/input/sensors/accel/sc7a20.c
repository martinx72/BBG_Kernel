...
{	
	...
	*chip_id = 0x0f;
	...
	result = sensor_write_reg(client, SC7A20_MODE, 0x07);  //sensor_enable(sensor, SENSOR_ON);
...
extern int avrman_sensor;
extern struct input_dev *snes_dev;
...
static int gsensor_report_value(struct i2c_client *client, struct sensor_axis *axis)
{
	...
	input_report_abs(sensor->input_dev, ABS_Z,-axis->z);
#else
	input_report_abs(snes_dev, ABS_X,axis->x);
	input_report_abs(snes_dev, ABS_Y,-axis->y);
	input_report_abs(snes_dev, ABS_Z,-axis->z);
#endif
