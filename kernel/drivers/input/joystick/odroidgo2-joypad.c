...
/*----------------------------------------------------------------------------*/
#define DRV_NAME "gamekiddy_joypad"
...
extern void pwm_vibrator_close( void );
...
static int joypad_adc_setup(struct device *dev, struct joypad *joypad)
{
	...
	for (nbtn = 0; nbtn < joypad->bt_adc_count; nbtn++) {
		...
		if (nbtn == 1 ) {
			adc->channel =
				devm_iio_channel_get(dev, "joy1_y");
			...
			adc->channel =
				devm_iio_channel_get(dev, "joy1_x");
			...
			adc->channel =
				devm_iio_channel_get(dev, "joy2_y");
			...
			adc->channel =
				devm_iio_channel_get(dev, "joy2_x");
...
extern int pwm_vibrator_play_effect(struct input_dev *dev, void *data, struct ff_effect *effect);

int joypad_input_ff( struct input_dev *input )
{
	int err;
