...
struct rk817_battery_device *avrman_battery = NULL;	

int avrman_rk817_bat_get_charge_state(void)
{
	union power_supply_propval val;
	int ret;
	struct power_supply *psy;

	psy = avrman_battery->bat;

	if (psy) {
		ret = psy->desc->get_property(psy, POWER_SUPPLY_PROP_STATUS,
					      &val);
		if (!ret)
		{
...
EXPORT_SYMBOL(avrman_rk817_bat_get_charge_state);
