...
static struct bq2589x *avrman_bq;
...
static int bq2589x_update_bits(struct bq2589x *bq, u8 reg, u8 mask, u8 data)
{
 ...
 enum bq2589x_vbus_type bq2589x_get_vbus_type(struct bq2589x *bq)
{
	u8 val = 0;
	int ret;
...
int bq2589x_get_charging_status( void )
{
	...
	ret = bq2589x_read_byte(avrman_bq, &val, BQ2589X_REG_0B);
	if (ret < 0) {
		dev_err(avrman_bq->dev, "%s Failed to read register 0x0b:%d\n", __func__, ret);
		return ret;
	}
...
void bq2589x_set_otg( int enable)
{
	...
	if (enable) {
		ret = bq2589x_enable_otg(avrman_bq);
		if (ret < 0) {
			dev_err(avrman_bq->dev, "%s:Failed to enable otg-%d\n", __func__, ret);
			return;
		}
	} else{
		ret = bq2589x_disable_otg(avrman_bq);
		if (ret < 0)
			dev_err(avrman_bq->dev, "%s:Failed to disable otg-%d\n", __func__, ret);
	}
}
...
#endif
	case POWER_SUPPLY_PROP_ONLINE:
		bq2589x_read_byte(bq, &type, BQ2589X_REG_0B);
		type &= BQ2589X_CHRG_STAT_MASK;
		type >>= BQ2589X_CHRG_STAT_SHIFT;	
			
		if ( type == BQ2589X_CHRG_STAT_FASTCHG)
			val->intval = 1;
		else if ( type == BQ2589X_CHRG_STAT_PRECHG)
			val->intval = 1;
		else
			val->intval = 0;
		break;
...
EXPORT_SYMBOL(avrman_get_online);

#include <linux/extcon.h>

extern struct extcon_dev *avrman_edev;
u8 ac_in;
u8 usb_in;
	
static void avrman_get_usb_status( void )
{

	struct extcon_dev *edev = avrman_edev;
...
static const struct power_supply_desc bq25890_usb_power_supply_desc = {
	.name = "bq2589x-usb",
	.type = POWER_SUPPLY_TYPE_USB,
...
/*
static const struct power_supply_desc bq25890_wall_power_supply_desc = {
	.name = "bq2589x-Wall",
	.type = POWER_SUPPLY_TYPE_MAINS,
	.properties = bq2589x_charger_props,
	.num_properties = ARRAY_SIZE(bq2589x_charger_props),
...
*/
	...
	bq->usb = power_supply_register(bq->dev, &bq25890_usb_power_supply_desc, NULL);
				    
	ac_psy = power_supply_register(bq->dev, &rk817_usb_desc, NULL);
	usb_psy = power_supply_register(bq->dev, &rk817_ac_desc, NULL);				   
	
//	bq->wall = power_supply_register(bq->dev, &bq25890_wall_power_supply_desc, NULL);
...
static void bq2589x_adapter_in_workfunc(struct work_struct *work)
{
	...
	if (bq->vbus_type == BQ2589X_VBUS_USB_DCP) {/* DCP, let's check if it is PE adapter*/
			   ...
			   const struct i2c_device_id *id)
{
	...
	irq = devm_gpiod_get(bq->dev, "bq25890_irq", GPIOD_IN);
	if (IS_ERR(irq)) {
	...
	otg =  devm_gpiod_get_optional(bq->dev, "avrman-otg", GPIOD_OUT_HIGH);
	
	if (!IS_ERR_OR_NULL( otg )) {
		printk("%s : otg %d\n", __func__,
		    desc_to_gpio( otg ) );
	}
	gpiod_set_value( otg ,1);
...
struct gpio_desc *charger_led1;

void avrman_led_init( int capacity )
{
	charger_led1 =  devm_gpiod_get_optional(avrman_bq->dev, "avrman-charger-led1", GPIOD_OUT_LOW);
